...
export const SideWidgets: React.FC<SideWidgetsProps> = ({
  ...
  return (
    <div id="side-widgets-container" className="lg:col-span-4 space-y-4 flex flex-col justify-start">
      {/* AI Match Panel */}
      <div className="bg-slate-900 text-white rounded-lg overflow-hidden shadow-lg border border-slate-800">
        <div className="p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            {selectedClass && hasApiKey && (
              <button onClick={onRunMatch} disabled={isMatchingLoading}
                className="px-2.5 py-1 bg-indigo-600/30 hover:bg-indigo-600 text-indigo-300 hover:text-white rounded-lg text-[10px] font-bold transition-all border border-indigo-500/30 flex items-center gap-1 cursor-pointer">
          ...
          {isMatchingLoading ? (
            <div className="p-4 text-center text-slate-400 text-xs flex flex-col items-center gap-2 bg-white/5 rounded-lg">
            ...
            <div className="space-y-2">
              {recommendedList.map((item, idx) => (
                <div key={idx} className={`flex items-center gap-3 p-3 rounded-lg ${idx === 0 ? 'bg-indigo-500/15 border border-indigo-500/20' : 'bg-white/5'}`}>
                  <div className={`w-8 h-8 rounded-lg ${item.tutor!.avatarColor || 'bg-indigo-500'} flex items-center justify-center font-bold text-white text-xs shrink-0`}>
                    {item.tutor!.avatar}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-indigo-300 mt-0.5 truncate italic">"{item.rationale}"</div>
                  </div>
                </div>
              ))}
            </div>
          ) : selectedClass && hasApiKey ? (
            <div className="bg-white/5 rounded-lg p-3 text-center text-xs text-slate-400">
      ...
      {topGS.length > 0 && (
        <div className="bg-white rounded-lg border border-slate-200/80 shadow-sm p-5">
          ...
          <div className="space-y-1.5">
            {topGS.map((gs, i) => (
              <div key={gs.code} className={`flex items-center gap-2.5 p-2 rounded-lg ${i === 0 ? 'bg-amber-50' : 'hover:bg-slate-50'} transition-colors`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-[9px] text-indigo-600 font-medium">{gs.activeCount} lớp</span>
