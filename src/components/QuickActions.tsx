...
export const QuickActions: React.FC<QuickActionsProps> = ({ tutors, registrations, matches, attendance, reviews, transactions = [], onNavigate }) => {
  ...
  const actions = [
    ...
    { show: unverifiedTutors > 0, color: 'amber', icon: <UserCheck className="w-4 h-4" />, label: `${unverifiedTutors} gia sư chờ xác minh`, sub: 'Duyệt hồ sơ gia sư', tab: 'tutors' as ActiveTab, urgent: false },
    { show: dormantTutors.length > 0, color: 'orange', icon: <ShieldAlert className="w-4 h-4" />, label: `${dormantTutors.length} gia sư "ngủ đông"`, sub: '>30 ngày không hoạt động', tab: 'tutors' as ActiveTab, urgent: false },
    ...
    { show: unpaidUrgent > 0 && unpaidCritical === 0, color: 'orange', icon: <DollarSign className="w-4 h-4" />, label: `${unpaidUrgent} lớp chưa thu phí >7 ngày`, sub: 'Gia sư cần nộp phí sớm', tab: 'matches' as ActiveTab, urgent: true },
    { show: unpaidFees > 0 && unpaidUrgent === 0, color: 'amber', icon: <DollarSign className="w-4 h-4" />, label: `${unpaidFees} lớp chưa thu phí kết nối`, sub: 'Gia sư nhận lớp >3 ngày', tab: 'matches' as ActiveTab, urgent: false },
    { show: overduePayments > 0, color: 'red', icon: <DollarSign className="w-4 h-4" />, label: `${overduePayments} gia sư nợ phí >28 ngày`, sub: 'Cần xử lý!', tab: 'finance' as ActiveTab, urgent: overduePayments > 2 },
