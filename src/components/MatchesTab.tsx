    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <div className="flex gap-2">
          <button onClick={exportCsv}
            className="px-3 py-2 bg-white border border-slate-200 hover:border-slate-300 rounded-lg text-xs font-bold text-slate-600 cursor-pointer flex items-center gap-1.5 transition-colors">
            <Download className="w-3.5 h-3.5" /> Xuất CSV
          </button>
          <button onClick={() => setShowAdd(true)}
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold cursor-pointer flex items-center gap-2 shadow-sm shadow-indigo-600/20 transition-colors">
          ...
          {(['all', 'Đang dạy', 'Hoàn thành', 'Hủy'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-2 rounded-lg text-xs font-semibold border cursor-pointer transition-all ${
                filter === f ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm' : 'bg-white text-slate-600 border-slate-200 hover:border-indigo-300'
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map(m => (
                  ...
                  <tr className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors last:border-0">
                    <td className="px-4 py-3">
                      <span className="font-mono font-bold text-indigo-600 text-xs">{m.classCode}</span>
                    ...
                    <td className="px-4 py-3 text-xs font-bold text-indigo-600 tabular-nums">{fmt(m.fee)}đ</td>
                    <td className="px-4 py-3 text-xs text-slate-500 tabular-nums">{fmtDate(m.startDate)}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${
                        m.status === 'Đang dạy' ? 'bg-emerald-50 text-emerald-700' :
                        m.status === 'Hoàn thành' ? 'bg-indigo-50 text-indigo-700' :
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-1">
                        {m.status === 'Đang dạy' && (
                          <>
                            <button onClick={() => m.id && onUpdateStatus(m.id, 'Hoàn thành')}
                              className="px-2 py-1 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-bold rounded-lg cursor-pointer flex items-center gap-1 transition-colors">
                              <CheckCircle2 className="w-3 h-3" /> Xong
                            </button>
                            <button onClick={() => m.id && onUpdateStatus(m.id, 'Hủy')}
                              className="px-2 py-1 bg-slate-100 hover:bg-red-50 text-slate-500 hover:text-red-600 text-[10px] font-bold rounded-lg cursor-pointer transition-colors">
                        ...
                        <button onClick={() => generateConnectionContract(m, centerName || 'Gia Sư Thành Đạt', centerPhone || '', bankInfo)}
                          ...
                          className="px-2 py-1 hover:bg-indigo-50 text-slate-400 hover:text-indigo-600 rounded-lg cursor-pointer transition-colors">
                          <Printer className="w-3 h-3" />
                        </button>
                        <button onClick={() => m.id && window.confirm(`Xóa ghép lớp ${m.classSubject} - ${m.tutorName}?`) && onDeleteMatch(m.id)}
                          className="px-2 py-1 hover:bg-red-50 text-slate-400 hover:text-red-600 rounded-lg cursor-pointer transition-colors">
                          <Trash2 className="w-3 h-3" />
                        </button>
                      </div>
                      <button onClick={() => setExpandedNotes(expandedNotes === m.id ? null : (m.id || null))}
                        className="px-2 py-1 hover:bg-indigo-50 text-slate-400 hover:text-indigo-600 rounded-lg cursor-pointer relative transition-colors mt-1">
                        <MessageSquare className="w-3 h-3" />
                        {(m.internalNotes || []).length > 0 && (
                          <span className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-indigo-600 text-white rounded-full text-[8px] flex items-center justify-center font-bold">{(m.internalNotes || []).length}</span>
                        )}
                      </button>
                    </td>
                  </tr>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {m.statusHistory.map((h, i) => (
                                  <div key={i} className="text-[10px] px-2 py-1 bg-white border border-slate-200 rounded-lg">
                                    <span className={`font-bold ${h.status === 'Đang dạy' ? 'text-emerald-600' : h.status === 'Hủy' ? 'text-red-600' : 'text-indigo-600'}`}>{h.status}</span>
                          ...
                          {onAddNote && (
                            <div className="flex gap-1.5 mt-2">
                              ...
                              <button onClick={() => {
                                ...
                                }} className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-bold rounded-lg cursor-pointer transition-colors">Gửi</button>
                              </div>
                          )}
                        </div>
                      </td>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg border border-slate-200/75 p-4 shadow-sm">
          ...
          <div className="text-2xl font-bold text-indigo-600 mt-1">{doneCount}</div>
        </div>
        <div className="bg-white rounded-lg border border-slate-200/75 p-4 shadow-sm">
        ...
        <div className="fixed inset-0 z-50 modal-backdrop flex items-center justify-center p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6 shadow-2xl border border-slate-200/75 animate-scale-in">
            ...
            <form onSubmit={handleCreate} className="space-y-3 text-sm">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">Chọn lớp *</label>
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">Chọn gia sư *</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">Tên học sinh</label>
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">SĐT phụ huynh</label>
                  <input value={parentPhone} onChange={e => setParentPhone(e.target.value)} placeholder="0912345678" type="tel"
                    className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:border-indigo-500 transition-colors" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">Phí/buổi (VNĐ)</label>
                <input type="number" value={fee} onChange={e => setFee(Number(e.target.value))} min={50000} step={10000}
                  className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:border-indigo-500 transition-colors" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">Buổi/tháng</label>
                  <input type="number" value={sessionsPerMonth} onChange={e => setSessionsPerMonth(Number(e.target.value))} min={1} max={30}
                    className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:border-indigo-500 transition-colors" />
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">Phí kết nối (%)</label>
              ...
              {fee > 0 && sessionsPerMonth > 0 && (
                <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-3 text-xs shadow-sm">
                  <div className="font-bold text-indigo-800">Phí kết nối 1 lần: <span className="text-indigo-600">{fmt(Math.round(fee * sessionsPerMonth * feePercent / 100))}đ</span></div>
                  <div className="text-indigo-600 mt-0.5">= {fmt(fee)}đ × {sessionsPerMonth} buổi × {feePercent}%</div>
                </div>
              )}
              <div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button type="submit"
                  className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold cursor-pointer transition-colors shadow-sm">Ghép lớp</button>
              </div>
            </form>
          </div>
        </div>
