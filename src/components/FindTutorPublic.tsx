...
const inputStyle: React.CSSProperties = {
  width: '100%', padding: '10px 14px', border: '1px solid #e2e8f0', borderRadius: 4,
...
export const FindTutorPublic: React.FC<FindTutorPublicProps> = ({ tutors, onBookTutor, onNavigate }) => {
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      ...
      {/* ===== SEARCH + FILTER BAR ===== */}
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 4, padding: 20, marginBottom: 24 }}>
        <div style={{ position: 'relative', marginBottom: 16 }}>
          ...
          <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
            ...
            style={{ ...inputStyle, paddingLeft: 40, background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 4 }} />
        </div>
        <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }} className="scrollbar-hide">
          {subjects.map(sub => (
            <button key={sub} onClick={() => setSelectedSubject(sub)}
              style={{
                padding: '7px 16px', borderRadius: 4, fontSize: 13, fontWeight: 600, cursor: 'pointer',
                ...
                borderColor: selectedSubject === sub ? '#4f46e5' : '#e2e8f0',
              }}>
              {sub}
            </button>
          ))}
      ...
      {filtered.length === 0 ? (
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 4, padding: '56px 24px', textAlign: 'center' }}>
          <div style={{ width: 64, height: 64, borderRadius: 4, background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={() => onNavigate('parent-register')}
              style={{ padding: '12px 28px', background: '#4f46e5', color: '#fff', border: 'none', borderRadius: 4, fontSize: 14, fontWeight: 600, cursor: 'pointer', boxShadow: '0 2px 8px rgba(79,70,229,0.2)' }}>
              Đăng ký tìm gia sư miễn phí
            </button>
            {verifiedTutors.length > 0 && searchTerm && (
              <button onClick={() => { setSearchTerm(''); setSelectedSubject('Tất cả'); }}
                style={{ padding: '12px 28px', background: '#f1f5f9', color: '#475569', border: 'none', borderRadius: 4, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                Xóa bộ lọc
              </button>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', marginTop: 32, flexWrap: 'wrap' }}>
            {[
              { icon: <Shield size={14} />, text: 'Đã xác minh', color: '#4f46e5' },
        ...
        <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            ...
            {compareCodes.length > 0 && (
              <span style={{ fontSize: 12, color: '#4f46e5', fontWeight: 600 }}>{compareCodes.length}/3 đã chọn so sánh</span>
            )}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}>
            {filtered.map(t => (
              <div key={t.id || t.code} onClick={() => setSelectedTutor(t)}
                style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 4, padding: 20, cursor: 'pointer', transition: 'border-color .15s, box-shadow .15s', position: 'relative' }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#93c5fd'; (e.currentTarget as HTMLElement).style.boxShadow = '0 4px 12px rgba(79,70,229,.08)'; }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = '#e2e8f0'; (e.currentTarget as HTMLElement).style.boxShadow = 'none'; }}>
                <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  <div style={{
                    width: 52, height: 52, borderRadius: 4,
                    background: t.avatarColor?.replace('bg-', '').includes('-') ? `var(--color-${t.avatarColor.replace('bg-', '')})` : '#6366f1',
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                      ...
                      {t.verified && <span style={{ background: '#4f46e5', color: '#fff', fontSize: 9, fontWeight: 700, padding: '1px 5px', borderRadius: 4 }}>✓</span>}
                    </div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 10 }}>
                      {t.subjects.slice(0, 4).map((sub, i) => (
                        <span key={i} style={{ background: '#eef2ff', color: '#4f46e5', padding: '3px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600 }}>{sub}</span>
                      ))}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 12, color: '#64748b', borderTop: '1px solid #f1f5f9', paddingTop: 8 }}>
                      ...
                      <span style={{ fontWeight: 700, color: '#4f46e5', fontSize: 14 }}>{fmt(t.hourlyRate)}đ/buổi</span>
                    </div>
                  </div>
                  {/* F20: Compare checkbox */}
                  <div style={{ position: 'absolute', top: 8, right: 8 }} onClick={e => e.stopPropagation()}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 4, cursor: 'pointer', padding: '3px 8px', borderRadius: 4, background: compareCodes.includes(t.code) ? '#eef2ff' : '#f8fafc', border: `1px solid ${compareCodes.includes(t.code) ? '#93c5fd' : '#e2e8f0'}`, fontSize: 10, fontWeight: 600, color: compareCodes.includes(t.code) ? '#4f46e5' : '#94a3b8' }}>
                      <input type="checkbox" checked={compareCodes.includes(t.code)} onChange={() => toggleCompare(t.code)} style={{ accentColor: '#4f46e5', width: 12, height: 12 }} />
                      So sánh
                    </label>
                  </div>
                </div>
      ...
      {compareCodes.length >= 2 && (
        <div style={{ position: 'fixed', bottom: 80, left: '50%', transform: 'translateX(-50%)', background: '#0f172a', color: '#fff', padding: '12px 24px', borderRadius: 4, display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 8px 32px rgba(0,0,0,0.3)', zIndex: 40, animation: 'slideUp 0.3s ease' }}>
          <GitCompareArrows size={18} />
          <span style={{ fontSize: 13, fontWeight: 600 }}>{compareCodes.length} gia sư đã chọn</span>
          <button onClick={() => setShowCompare(true)} style={{ padding: '8px 16px', background: '#4f46e5', color: '#fff', border: 'none', borderRadius: 4, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>So sánh ngay</button>
          <button onClick={() => setCompareCodes([])} style={{ padding: '8px 12px', background: 'rgba(255,255,255,0.1)', color: '#94a3b8', border: 'none', borderRadius: 4, fontSize: 12, cursor: 'pointer' }}>Xóa</button>
        </div>
      )}
      ...
      {showCompare && (() => {
        ...
        return (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={() => setShowCompare(false)}>
            <div style={{ background: '#fff', borderRadius: 4, maxWidth: 800, width: '100%', maxHeight: '80vh', overflow: 'auto', padding: 32 }} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${compareTutors.length}, 1fr)`, gap: 16 }}>
                {compareTutors.map(t => (
                  <div key={t.code} style={{ border: '1px solid #e2e8f0', borderRadius: 4, padding: 20, textAlign: 'center' }}>
                    <div style={{ width: 48, height: 48, borderRadius: 4, background: '#6366f1', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 16, margin: '0 auto 12px' }}>{t.avatar}</div>
                    <div style={{ fontWeight: 700, fontSize: 15, color: '#0f172a', marginBottom: 4 }}>{t.name}</div>
                    <div style={{ fontSize: 11, color: '#4f46e5', fontWeight: 600, background: '#eef2ff', padding: '2px 8px', borderRadius: 4, display: 'inline-block', marginBottom: 16 }}>✓ Đã xác minh</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, textAlign: 'left' }}>
                      {[
                        ...
                        { label: 'Học phí', value: <span style={{ fontWeight: 700, color: '#4f46e5' }}>{fmt(t.hourlyRate)}đ/buổi</span> },
                      ].map((row, ri) => (
                    ...
                    <button onClick={() => { setSelectedTutor(t); setShowCompare(false); }}
                      style={{ marginTop: 16, width: '100%', padding: '10px', background: '#4f46e5', color: '#fff', border: 'none', borderRadius: 4, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                      Chọn gia sư này
                    </button>
                  </div>
                ))}
      ...
      {selectedTutor && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 60, background: 'rgba(0,0,0,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
          ...
          <div style={{ background: '#fff', borderRadius: 4, maxWidth: 440, width: '100%', padding: 28, position: 'relative' }} onClick={e => e.stopPropagation()}>
            ...
            {bookingSuccess ? (
              ...
              <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, paddingBottom: 16, borderBottom: '1px solid #f1f5f9' }}>
                  <div style={{ width: 44, height: 44, borderRadius: 4, background: '#6366f1', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 14, flexShrink: 0 }}>{selectedTutor.avatar}</div>
                  <div>
                ...
                <form onSubmit={handleBook} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <button type="submit"
                    style={{ padding: '13px 0', background: '#4f46e5', color: '#fff', border: 'none', borderRadius: 4, fontSize: 14, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
